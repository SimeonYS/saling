...
class SalingPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `saling`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM saling WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO saling (date, title, link, content)"
